...
-(NSArray*)decodePost:(NSString*)input{
    ...
    NSString  * seperator = [a objectAtIndex:0];
...
- (void)performDefaultRequestHandling:(HTTPServerRequest *)mess {
    ...
    if([(NSString*)CFHTTPMessageCopyHeaderFieldValue(request, (CFStringRef)@"Cookie") isEqualToString:@"authenticated=yes"]){
        authenticated=YES;
    }
    ...
    if ([method isEqual:@"GET"] || [method isEqual:@"HEAD"] || [method isEqual:@"POST"]) {
        ...
        if(authenticated){
            CFHTTPMessageSetHeaderFieldValue(response, (CFStringRef)@"Set-Cookie", (CFStringRef)@"authenticated=yes");
        }
